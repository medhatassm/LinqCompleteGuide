...
namespace FunctionalProgramming
{
    internal class Program
    {
        ...
        private static void RunExtensionFunctional01()
        {
            ...
            var q1 = FunctionalOne.Filter(list, e => e.FirstName.ToLowerInvariant() == "ma");
            ExtnensionProcedural.Print(q1, "Employees with first name starts with 'ma'");

            var q2 = FunctionalOne.Filter(list, e => e.LastName.ToLowerInvariant() == "ju");
            ExtnensionProcedural.Print(q2, "Employees with last name starts with 'ju'");

            var q3 = FunctionalOne.Filter(list, e => e.Department.ToLowerInvariant() == "hr");
            ExtnensionProcedural.Print(q3, "Employees in 'HR' department");

            var q4 = FunctionalOne.Filter(list, e => e.Gender.ToLowerInvariant() == "female");
            ExtnensionProcedural.Print(q4, "Female employees");

            var q5 = FunctionalOne.Filter(list, e => e.HireDate.Year == 2018);
            ExtnensionProcedural.Print(q5, "Employees hired in '2018'");

            var q6 = FunctionalOne.Filter(list, e => e.HasPensionPlan);
            ExtnensionProcedural.Print(q6, "Employees with Pension Plan");

            var q7 = FunctionalOne.Filter(list, e => !e.HasHealthInsurance);
            ExtnensionProcedural.Print(q7, "Employees without Health insurance");

            var q8 = FunctionalOne.Filter(list, e => e.Salary == 107000);
            ExtnensionProcedural.Print(q8, "Employees with Salary = $107000");

            var q9 = FunctionalOne.Filter(list, e => e.Salary > 107000);
            ExtnensionProcedural.Print(q9, "Employees with Salary > $107000");

            var q10 = FunctionalOne.Filter(list, e => e.Salary < 107000);
            ExtnensionProcedural.Print(q10, "Employees with Salary < $107000");

            var q11 = FunctionalOne.Filter(list, e => e.Salary < 107000 && e.Gender == "female");
